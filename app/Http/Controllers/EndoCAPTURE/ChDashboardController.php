...
class ChDashboardController extends Controller
{
    ...
    public function index(Request $r)
    {
        ...
        return view('Dashboard02.main',$view);
    ...
    public function request_filter(Request $r){
        ...
        return redirect(url('chdashboard'));
    }
    ...
    public function get_treatment_coverage($cases, $r){
        $arr = $this->get_casedata('treatment_coverage', $cases);
        $treamentcount = $arr['treatment_coverage'];
        $treatment = $treamentcount ?? [];
    ...
    public function get_patient_count($gender, $age_from, $age_to, $wc){
        ...
        $patient = $this->get_casedata('gender', $wc);
        $count = 0;
        if(isset($patient['tb_case'])){
            $count   = count($patient['tb_case']) ?? 0;
        }
        return $count;
        // return 0;
    }
    ...
    public function get_physician($cases, $r){
        ...
        $users = $this->get_staff_count('doctor', $r->department, true, false);
        $physician = [];
        foreach ($users as $user) {
            $user = (object) $user;
            if(!isset($user)){
                continue;
            } else if(!isset($user->user_firstname)){
                continue;
            }
