...
class PhotomoveController extends Controller
{
    public function show(Request $r, $id){
        ...
        $view['video_all']  = isset($view['case']->video) ? $view['case']->video : [];
        $case_hn = isset($view['case']->case_hn) ? $view['case']->case_hn : (isset($view['case']->hn) ? $view['case']->hn : '');
        $view['vdo_url']    = domainname('store') . "/" . $case_hn . "/" . $view['folderdate'] . "/vdo";
        $view['cid']        = $id;
        $view['otp']        = rand(1000,9999);
        $view['user_id']    = uid();
        return view('capture.photomove.newshow',$view);
    ...
    public function photo_move_case($r){
            ...
            // dd($r->all());
            $tb_case_new = Mongo::table('tb_case')->where('_id',$cid_new)->first();
            $hn_new = $tb_case_new->case_hn;
            ...
            if($r->photoname != null){
                foreach($r->photoname as $photo){
                    ...
                    $ex = explode("_",$photo);
                    if(isset($ex[4])){
                        $photoname_new = $cid_new."_".$ex[1]."_".$hn_new."_".$ex[3]."_".$ex[4];
                    }else{
                        $photoname_new = $cid_new."_".$ex[1]."_".$hn_new."_".$ex[3];
                    }
                ...
                $val['photo'] = $photo_arr;
                Mongo::table('tb_case')->where('id',$cid)->update($val);
            }

            // Move videos
            if($r->videoname != null && is_array($r->videoname)){
                $tb_case_new = Mongo::table('tb_case')->where('_id',$cid_new)->first();
                $appointment_new = isset($tb_case_new->appointment) ? $tb_case_new->appointment : $tb_case_new->appointment_date;
                $folderdate_new = str_contains($appointment_new, ' ') ? explode(" ", $appointment_new)[0] : $appointment_new;
                $hn_new = isset($tb_case_new->case_hn) ? $tb_case_new->case_hn : (isset($tb_case_new->hn) ? $tb_case_new->hn : '');

                foreach($r->videoname as $video){
                    $ex = explode("_",$video);
                    if(isset($ex[4])){
                        $videoname_new = $cid_new."_".$ex[1]."_".$hn_new."_".$ex[3]."_".$ex[4];
                    }else{
                        $videoname_new = $cid_new."_".$ex[1]."_".$hn_new."_".$ex[3];
                    }

                    $old_video_path = htdocs("store/$hn/$folderdate/vdo/$video");
                    $new_video_dir = htdocs("store/$hn_new/$folderdate_new/vdo");
                    $new_video_path = "$new_video_dir/$videoname_new";

                    if(file_exists($old_video_path)){
                        makedirfull($new_video_dir);
                        if(rename($old_video_path, $new_video_path)){
                            // Add video to new case
                            $tb_case_new = Mongo::table('tb_case')->where('_id',$cid_new)->first();
                            $video_all = isset($tb_case_new->video) ? $tb_case_new->video : [];
                            if(!is_array($video_all)){
                                $video_all = [];
                            }
                            $video_all[] = $videoname_new;
                            $val_video['video'] = $video_all;
                            Mongo::table('tb_case')->where('_id',$cid_new)->update($val_video);
                        }
                    }
                }

                // Remove videos from old case
                $tb_case_old = Mongo::table('tb_case')->where('_id',$cid)->first();
                $video_all_old = isset($tb_case_old->video) ? $tb_case_old->video : [];
                if(!is_array($video_all_old)){
                    $video_all_old = [];
                }
                $video_arr_new = [];
                foreach($video_all_old as $v){
                    if(!in_array($v, $r->videoname)){
                        $video_arr_new[] = $v;
                    }
                }
                $val_video_old['video'] = $video_arr_new;
                Mongo::table('tb_case')->where('_id',$cid)->update($val_video_old);
            }

            $case_old = Mongo::table('tb_case')->where('id',$cid)->first();
            $case_new = Mongo::table('tb_case')->where('id',$cid_new)->first();
            ...
            $data['edit_status']    = 1;
            $data['edit_json']      = $js;
            $event_name = 'photo move';
            if($r->videoname != null && is_array($r->videoname) && count($r->videoname) > 0){
                $event_name = 'photo & video move';
            }
            $data['edit_event']     = $event_name;
