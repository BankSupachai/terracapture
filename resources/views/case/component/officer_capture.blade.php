    ...
    <div class="card card-custom gutter-b">
        <div class="card-body">
            <div class="row">
                <div class="col-12">
                    <h3 class="card-label">
                        <u>Possible Colorectal Cancer</u>
                    </h3>
                ...
                <div class="col-4 mt-1">
                    <input @checked(@$case->possiblecolorectalcancer == 'no') id="box_rapid_pending"
                        name="possiblecolorectalcancer" value="no" type="radio"
                        class="form-check-input possiblecolorectalcancer">
                    <span></span>
                    <label class="form-check-label" for="box_rapid_pending"> &nbsp; NO</label>
                </div>
                <div class="col-4 mt-1">
                    <input @checked(@$case->possiblecolorectalcancer == 'yes') id="box_rapid_positive"
                        name="possiblecolorectalcancer" value="yes" type="radio"
                        class="form-check-input possiblecolorectalcancer">
                    <span></span>
                    <label class="form-check-label" for="box_rapid_positive"> &nbsp; YES</label>
                </div>
...
<div class="col-12 p-0">
            ...
            <div class="row">
                <div class="col-xxl-6">
                    ...
                    <div class="row">
                        <div class="col-11 pe-0">
                            <select name="case_physicians01" id="case_physicians01" class="form-control" required select2>
                                ...
                                @foreach ($doctor as $data)
                                    <option value="{{ $data['id'] }}" @selected($case->case_physicians01 == $data['id'])>
                        ...
                        <div class="col-12 mt-2">
                            Assistant<br>
                            <textarea id='assistant' name="assistant" type="text" placeholder="Free text" class="savejson form-control autotext">{{ @$case->assistant }}</textarea>
                        </div>
                ...
                <div class="col-xxl-6">
                    <div class="row">
                        <div class="col-12">
                            Attendant
                            <select class="form-select" name="attendant" id="select_users">
                                <option value="0">Select Users</option>
                                @php
                                    $lists = [];
                                    if (isset($case->user_in_case)) {
                                        $lists = $case->user_in_case;
                                        $user_list = Mongo::table('users')->whereIn('id', $lists)->get();
                                    }
                                @endphp
                                @if (isset($users))
                                    @foreach ($users as $index => $data)
                                        @php
                                            $data = (object) $data;
                                            $is_attendant = in_array($data->id, $lists) ? 'true' : 'false';
                                        @endphp
                                        <option value       ="{{ @$data->id }}" data-tokens ="{{ @$data->id }}"
                                            data-name   ="{{ @$data->user_prefix }} {{ @$data->user_firstname }} {{ @$data->user_lastname }}"
                                            data-type   ="{{ @$data->user_type }}" data-tab    ="{{ @$data->id }}"
                        ...
                        <div class="col-12 mt-2">
                            ...
                            <div class="row m-0 scroll-list" id="scroll_list">
                                ...
                                @foreach (isset($case->user_in_case) ? $case->user_in_case : [] as $data)
                                    ...
                                    <div class="col-3 pt-2" sub-tab="{{ @$data }}">{{ @$user->user_type }}
                                    </div>
    ...
    <div class="col-12 text-center mb-3">
        @if (getCONFIG('feature')->system_esign)
            <button type="submit" name="report" value="true" id="btn_save" class="btn btn-success fs-16 "
                style="width: 50em;">
                <i class="ri-dossier-fill"></i>
                <span class="">Create Report</span>
            </button>
        @else
            <button type="submit" id="btn_save_hide" name="report" value="true" class="btn btn-success fs-16"
                style="width: 50em;">
                <i class="ri-dossier-fill "></i>
                <span class="">Create Report</span>
            </button>
