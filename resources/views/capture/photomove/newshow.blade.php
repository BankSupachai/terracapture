...
<style>
    ...
    .video-container.video-selected {
        background: #f06548;
        padding: 10px;
        margin: 10px;
    }
...
<div class="row m-0 p-3">
    <div class="bg-sortphoto">
        <div class="col-12 d-flex justify-content-between py-3">
            <div>
                <span class="text-sort-blue h3">Move Photo & Video</span>
                <p class="text-sort-blue">Select photo/video by "click" into photo/video and then select case by click "Select"
                </p>
            </div>
            <div>
                ...
                <div class="row draggable-zone">
                    <div class="col-12">
                        <div class="row text-center">
                            <form id="form_photo_move" action="{{ url('photomove') }}" class="row"
                                method="post">
                                @csrf
                                <input type="hidden" name="cid" value="{{ $cid }}">
                                <input id="cid_new" type="hidden" name="cid_new" value="">
                                <input type="hidden" name="hn" value="{{ @$case->case_hn ?? @$case->hn }}">
                                        ...
                                        <li class="col-lg-2 card card-custom gutter-b li-img draggable p-2"
                                            ...
                                            <label for="check0{{ $photo['nu'] }}" class="m-0 photo-hover">
                                                <img photo_id="{{ $photo['nu'] }}"
                                                    ...
                                                    src="{{ mePHOTO(@$case->case_hn ?? @$case->hn, $photo['na'], $folderdate) }}?a={{ RandomString() }}">
                                            </label>
                                        </li>
                                    @endforeach
                                    @foreach ($video_all as $index => $video)
                                        <li class="col-lg-2 card card-custom gutter-b li-img draggable p-2"
                                            style="background: transparent;">
                                            <input name="videoname[]" type="hidden"
                                                id="check_vdo{{ $index }}" class="checkbox_video"
                                                value="">
                                            <div class="video-container video-select-container"
                                                style="cursor: pointer;"
                                                data-video-index="{{ $index }}"
                                                data-video-name="{{ $video }}">
                                                <div class="video-inner" style="position: relative; width: 100%; padding-top: 75%; background: #000;">
                                                    <video
                                                        class="video_select w-100 h-100"
                                                    ...
                                                    <div class="video-overlay" style="position: absolute; top: 5px; left: 5px; background: rgba(0,0,0,0.7); color: white; padding: 2px 5px; border-radius: 3px; font-size: 10px; pointer-events: none;">
...
<script src="{{ url('public/extra/photomove/jquery.min.js') }}"></script>
    ...
    <script>
        ...
        $('#btn_photo_move').click(function() {
            ...
            if (ck_text.length > 3) {
                if (otp == txt_otp && submit_status) {
                    // Remove empty video inputs before submit
        ...
        // Video selection without checkbox
        $('.video-select-container').click(function() {
            var video_index = $(this).data('video-index');
            var checkbox = $('#check_vdo' + video_index);
            var isSelected = checkbox.val() !== '';

            if (isSelected) {
                // Deselect
                checkbox.val('');
                $(this).removeClass('video-selected');
            } else {
                // Select
                checkbox.val($(this).data('video-name'));
        ...
        $('#photo_select_all').click(function() {
            ...
            // Select all videos
            $('.video-select-container').each(function() {
                var checkbox = $('#check_vdo' + $(this).data('video-index'));
                checkbox.val($(this).data('video-name'));
                $(this).addClass('video-selected');
            });
        })
