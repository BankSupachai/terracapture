...
<head>
    ...
    <style>
        ...
        .preview-container {
            ...
            align-items: center;
            height: 80vh; /* หรือ 100vh ถ้าอยากเต็มจอ */
            min-height: 700px;
            position: relative;
        ...
        .row>* {
            padding-right: calc(var(--vz-gutter-x) * 0);
            padding-left: calc(var(--vz-gutter-x) * 0);
        }
    </style>
</head>
...
<body>
    <div class="row" style="margin:0 !important;">
        <div class="col-2">
            <div class="photo-container">
                <div class="photo-header mt-2"><b>Photo & Video</b> ({{ count(($tb_case->photo??[])) + count(($tb_case->video??[])) }})</div>
                <div id="photoList">
                @php $idx = 1; @endphp
        ...
        <div class="col-10" style="overflow-y: auto; background-color: #343434;">
            <div class="row" style="margin-bottom:0; width:1575px;">
                <div class="zoom-controls" id="zoomControls" style="display: flex; flex-direction: row; justify-content: center; align-items: flex-end; ">
                    <button id="moveBtn" onclick="moveMode()" style="background: #707070; margin:10px; height: 60px; width: 80px; line-height: 1.1;">
                        <i class="ri-drag-move-2-line" style="font-size: 1.4em;"></i>
                        <span style="font-size: 0.7em;">Move</span>
                    </button>
                    <button onclick="zoomIn()"style="background: #707070; margin:10px; height: 60px; width: 90px; line-height: 1.1;">
                        <i class="ri-zoom-in-line" style="font-size: 1.4em;"></i>
                        <span style="font-size: 0.65em;">Zoom In</span>
                    </button>
                    <button onclick="zoomOut()" style="background: #707070; margin:10px; height: 60px; width: 90px; line-height: 1.1;">
                        <i class="ri-zoom-out-line" style="font-size: 1.4em;"></i>
                        <span style="font-size: 0.53em;">Zoom Out</span>
                    </button>
                    <button onclick="resetZoom()" style="background: #707070; margin:10px; height: 60px; width: 80px; line-height: 1.1;">
                        <i class="ri-refresh-line" style="font-size: 1.4em;"></i>
                        <span style="font-size: 0.7em;">Reset</span>
                    </button>
                </div>
            </div>
            <div class="row" style="width:1565px;">
                <div class="preview-container" style="margin-top:-8px">
                    ...
                    <h4 id="noMediaMessage" style="font-size: 1.5rem;">Select a media to preview</h4>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        // Auto-load first media when page loads
        document.addEventListener('DOMContentLoaded', function() {
            const firstMediaItem = document.querySelector('.media-item.photo-thumb[data-idx="1"]');
            if (firstMediaItem) {
                // Get the onclick attribute and extract the parameters
                const onclickAttr = firstMediaItem.getAttribute('onclick');
                if (onclickAttr) {
                    // Extract the parameters from the onclick string
                    const match = onclickAttr.match(/showPreviewAndHighlight\('([^']+)',\s*'([^']+)',\s*(\d+)\)/);
                    if (match) {
                        const mediaUrl = match[1];
                        const type = match[2];
                        const idx = parseInt(match[3]);
                        showPreviewAndHighlight(mediaUrl, type, idx);
                    }
                }
            }
        });
