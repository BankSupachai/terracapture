...
@endphp
<!DOCTYPE html>
<html lang="en" data-layout="horizontal" data-topbar="light" data-sidebar="dark" data-sidebar-size="{{ $pagesize }}"
...
<head>
    ...
    <style>
        @font-face {
            ...
            src: url("{{ url('public/fonts/Noto_Sans_Thai/static/NotoSansThai-Regular.ttf') }}") format('truetype');
            font-weight: normal;
            font-style: normal;
        }
        ...
        @font-face {
            font-family: 'Noto Sans Thai';
            src: url("{{ url('public/fonts/Noto_Sans_Thai/static/NotoSansThai-Bold.ttf') }}") format('truetype');
            font-weight: bold;
            font-style: bold;
        }
        ...
        * {
        ...
        /* Style สำหรับ search และ filter section */
        ...
        .date-range-segmented .date-option {
            flex: 1;
            padding: 8px 12px;
            text-align: center;
            cursor: pointer;
        ...
        .date-range-segmented .date-option.active {
            background: #192D4B;
            color: #ffffff;
        }

        .date-range-segmented .date-option:hover:not(.active) {
            background: rgba(25, 45, 75, 0.1);
        }
        ...
        .btn-create-worklist:hover {
            background: #1a3a5f;
            color: #ffffff;
        }

        .btn-create-worklist i {
            font-size: 16px;
            margin-right: 4px;
        }
    ...
    @include('EndoINDEX.Component.capture_css_main')
                    ...
                    <div class="d-flex">
                        <div class="navbar-brand-box horizontal-logo " style="margin-left:-90px;">
                            <a href="{{ url('home') }}" class="logo logo-dark">
                                ...
                                <span class="logo-lg">
                                    <img src="{{ url('public/image/TERRA.png.') }}" alt="" height="120">
                                </span>
                            </a>
                            ...
                            <div class="dropdown-menu dropdown-menu-end">
                                ...
                                @if (@$feature->adfs)
                                    <a id="btn_logout" href="{{ url('api/adfs/logout/edit') }}"
                                        ...
                                        <span class="align-middle" data-key="t-logout">Sign Out</span>
                                    </a>
                                @else
                                    <a class="dropdown-item auto-logout" href="{{ route('logoutcapture') }}"
                                        onclick=" event.preventDefault(); document.getElementById('logout-form').submit();"><i
                                            class="mdi mdi-logout text-muted fs-16 align-middle me-1"></i> <span
                                            class="align-middle" data-key="t-logout">Sign Out</span></a>
                                    <form id="logout-form" action="{{ route('logoutcapture') }}" method="POST"
                                        style="display: none;">
                                        @csrf
                                        <input type="hidden" name="userid" value="{{ @uid() }}">
                                    </form>
                                    {{-- <a class="dropdown-item auto-logout" id="closeTabButton"><i
                                            class="mdi mdi-exit-to-app text-muted fs-16 align-middle me-1"></i> <span
                                            class="align-middle" data-key="t-logout">Exit</span></a> --}}
                                @endif

                                <script>
                                    document.getElementById('closeTabButton').addEventListener('click', function() {
                                        // alert('ปิดแท็บแล้ว');
                                        // ลองใช้ window.close() เพื่อปิดแท็บปัจจุบัน
                                        window.close();

                                        // หากเบราว์เซอร์บล็อกการปิดแท็บ อาจจะไม่มีอะไรเกิดขึ้น หรือมีข้อความแจ้งเตือนใน Console
                                        // สำหรับบางกรณี อาจจะต้องลองใช้ history.back() หรือ location.href = "about:blank";
                                        // แต่อาจไม่ปิดแท็บจริง ๆ แค่เปลี่ยนหน้า
                                        // console.log("พยายามปิดแท็บแล้ว");
                                    });
                                </script>
...
$("li").on("mouseover", function () {
    ...
    let $menutext = $(".menu-hover1").text();
$(document).ready(function(){
    $(".nav-item").hover(function(){
       let menu_title =  $(this).attr("menu_title");
