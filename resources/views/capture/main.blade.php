...
@section('content')
    ...
    @php
        ...
        } elseif (isset($tb_case->patientname)) {
        ...
        <div class="col-12 my-2 fs-14">
            <div class="card">
                <div class="card-body">
                    ...
                    @endphp
                    <label class="text-terralink ps-3 pb-2">Patient Detail : </label>&emsp;
                    @isset($patientname)
                        @if ($patientname != '')
                        ...
                        @endif
                    @endisset
                    <label class="text-terralink">
                        @isset($gender)
                            {{ $gender == 1 ? 'Male' : 'Female' }}
                        @endisset
                    </label>&emsp;
                    <label class="text-terralink">{{ @$dob }} ({{ @$age }})</label>
                    <label class="text-terralink">Number of Study : <span
                            id="study_num">{{ count($tb_case) }}</span></label>&emsp;

                    <div class="row m-0">
                        <div class="col-2">
                            <select name="" class="form-select js-example-basic-single " id="procedure"
                                onchange="function_search()">
                            ...
                            <select id="physician" class="form-select js-example-basic-single" onchange="function_search()">
                                ...
                                @foreach (isset($doctor) ? $doctor : [] as $doc)
                                    @php
                                        $doc = (object) $doc;
                                        $fullname =
                                            @$doc->user_prefix . @$doc->user_firstname . ' ' . @$doc->user_lastname;
                                    @endphp
                        ...
                        <div class="col-2">
                            @php
                                $now = date('Y-m-d');
                            @endphp
                            <input type="text" placeholder="Select date range" class="form-control form-control-dark"
                                id="datepicker" />
                        ...
                        <div class="col">
                            <button type="button" onclick="compare_detail()" class="btn btn-compare " disabled><i
                                    class="ri-file-copy-2-fill"></i> Compare</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="offcanvas-header">
            <div class="row">
                <div class="col ">
                    ...
                    <span class="h4 " id="offcanvasRightLabel">Viewer</span>
                </div>
            </div>
            <span class="x-forclose" data-bs-dismiss="offcanvas" style="color:black">
                X
            </span>
        </div>
        <div class="col-12 p-0 m-0 border-under"></div>

        @include('capture.viewer.offcanvas')

        <!-- left offcanvas -->
        <div class="offcanvas offcanvas-start w-40 bg-terralink" tabindex="-1" id="offcanvasLeft"
            ...
            @include('capture.viewer.offcanvasleft')
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js"></script> --}}
            ...
            <script>
                    ...
                    $('#tasksTable .tr-row').each((index, element) => {
                        ...
                        if (range != '' && range != undefined) {
                            if (range == 'all') {
                                ...
                                if (now.includes(date_div)) {} else {
                                    $(element).hide()
                                }
                            } else if (range == 'week') {
                                var week = get_current_week()
                                if (week.includes(date_div)) {} else {
                                    $(element).hide()
                                }
                                console.log(week);
                            }
                        ...
                        if (daylist.length != 0) {
                            if (daylist.includes(date_div)) {} else {
                                $(element).hide()
                            }
                        }
                ...
                function change_pdf(num_id) {
                    ...
                    console.log(id, num_id, "mosssssssssssss");
                    // call_alert('load')
                ...
                function open_detail(hn, case_id, date, is_endosmart, num_id = '', compare) {
                    ...
                    $.post('{{ url('api') }}/case', {
                        ...
                        if (photo != null && photo != undefined && is_endosmart == 'false') {
                            photo.forEach(p => {
                                if (p['ns'] > 0) {
                                    var img_src = is_endosmart == 'true' ? `` :
                                        `{{ @$store_url }}${hn}/${date}/${p['na']}`
