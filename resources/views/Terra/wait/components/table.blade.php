...
<div class="row m-0 p-0 table-list">
    <table class="table table-nowrap table-borderless text-light">
        <thead>
            <tr style="background: #225381; border: 0 px">
                <td></td>
        ...
        <tbody id="tasksTable" class="" >
            @foreach (isset($tb_case) ? $tb_case : [] as $index => $case)
                ...
                @endphp
            <tr class="tr-row table_row" id="tr{{@$_id}}" data-hn="{{ @$hn }}" data-id="{{ @$_id }}" data-date="{{ @$date_only }}" data-endosmart="{{ @$is_endosmart }}"  >
                {{-- data-bs-toggle="offcanvas" data-bs-target="#offcanvasRight" aria-controls="offcanvasRight" --}}
                {{-- onclick="open_detail('{{ @$hn }}', '{{ @$_id }}', '{{ @$date_only }}', '{{ @$is_endosmart }}')" --}}
                <td>
                    <div class="col-auto">
                        <input class="form-check-input compare" type="checkbox" id="compare{{$_id}}" data-id="{{ @$_id }}">
                    </div>
                </td>
                <td class="open-offcanvas" data-id="{{ @$_id }}">{{ @$date }}</td>
                <td class="sort-procedure open-offcanvas" data-id="{{ @$_id }}">{{ @$procedurename }}</td>
                <td class="sort sort-physician open-offcanvas" data-id="{{ @$_id }}">{{ @$physician }}</td>
                <td class="open-offcanvas" data-id="{{ @$_id }}">{{ @$count_selected }}</td>
                <td class="open-offcanvas" data-id="{{ @$_id }}">@if(isset($video)) {{ count($video) }} @endif</td>
                ...
                <td class="sort-prediag open-offcanvas" data-id="{{ @$_id }}">{{ @$prediag }}</td>
