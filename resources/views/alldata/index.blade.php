        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <form action="{{ url('alldata') }}" method="GET">
                        @csrf
                        ...
                        <div class="row m-0">
                            ...
                            <div class="col-2">
                                <input type="text" class="form-control search-input" name="keyword" id="keyword" placeholder="ค้นหา" value="{{ @$_GET['keyword'] }}">
                            </div>
                            ...
                            <div class="col-2">
                                ...
                                <select name="procedure" id="procedure" class="form-select mb-3 search-input" onchange="get_icd(this.value)">
                                    <option value="">&nbsp;Procedure</option>
                                    @isset($procedure)
                                        @foreach ($procedure as $p)
                                            <option value="{{ @$p->code }}"
                                                @selected(@$_GET['procedure'] == @$p->code)>
                                                {{ @$p->name }}
                                            </option>
                                        @endforeach
                                    @endisset
                            ...
                            <div class="col-auto ps-2 pe-2">
                                <a id="clear_btn" href="{{ url('alldata') }}"
                            ...
                            <div class="col-auto ps-2 pe-2">
                                <a id="prepare_btn"
                                    class="btn btn-info waves-effect waves-light" style="width:100%">
                                    <i class="ri-eraser-fill label-icon align-middle " style=""></i> <span
                                        class="align-middle"> Update Data</span></a>
                                <script>
                                    $("#prepare_btn").click(function() {
                                        $.ajax({
                                            url: "{{ url('alldata') }}",
                                            type: "POST",
                                            data: {
                            ...
                            <div class="col-auto">
                                <div class="row">
                                    <div class=" text-end">
                                        <a class="btn btn-success yourlink pe-2 "><i class="ri-download-line ri-lg"></i>Download</a>
                                        @php
                                            $parameters = $_GET;
                                            ...
                                            $(".yourlink").click(function() {
                                                var urls = [
                                                    @for ($i = 0; $i < $download; $i++)
                                                        @php
                                                            $url = url("")."/alldata/$i/edit?$parameters";
                                                            echo "'$url',";
                                                        @endphp
                                                    @endfor
                                                ]
                ...
                <div style="height: 886px; overflow:auto">
                    <table class="table table-bordered ">
                        ...
                        @foreach ($tb_case ?? [] as $index => $case)
                            ...
                            @php
                                // $case = jsonDecode($case);
                                $case = jsonDecode($case->text);
                                ...
                                foreach (@$scope as $value) {
                                    // dd($value);
                                    try {
                                    $tb_scope = Mongo::table('tb_scope')->where('scope_id', intval($value))->first();
                                        if($tb_scope){
                                            $scopename[] = @$tb_scope->scope_name ?? "";
                                            $serialarr[] = @$tb_scope->scope_serial ?? "";
                                        }
                                    } catch (\Throwable $th) {
                                        $scopename[] = '-';
                                        $serialarr[] = '-';
